...
# show how many valid dates remain
...
# drop rows with NaN in dateoftransfer
...
# evaluate
xgb_rmse = mean_squared_error(y_test_xgb, y_xgb_pred) ** 0.5
xgb_mae = mean_absolute_error(y_test_xgb, y_xgb_pred)
xgb_r2 = r2_score(y_test_xgb, y_xgb_pred)
...
y_xgb_pred_vals = np.array(y_xgb_pred).astype(float)

plt.figure(figsize=(10, 6))
plt.scatter(y_test_vals, y_xgb_pred_vals, alpha=0.6, color='purple')
plt.plot(
    [y_test_vals.min(), y_test_vals.max()],
    [y_test_vals.min(), y_test_vals.max()],
    'r--', lw=2
)
...
ax.set_xlim(max(0, x_min - pad), x_max + pad)
ax.set_ylim(max(0, min(y_xgb_pred_vals.min(), x_min) - pad), max(y_xgb_pred_vals.max(), x_max) + pad)

plt.xlabel("Actual Prices (£)")
plt.ylabel("Predicted Prices (£)")
plt.title("Actual vs Predicted Property Prices (XGBoost)")
plt.tight_layout()
plt.show()
